...
| 6 | CRM-Tabelle | ✅ | 2026-05-12 | 014, 015 | TanStack-Table mit 10 Spalten (Name/Firma/Funktion/Tel/Email/Letzter Kontakt/Status/Lead-Herkunft/Deals/Notizen). Inline-Edit für alle Cells (Pattern aus Schritt 3 wiederverwendet, inkl. Status-Inline-Edit via EditableSelectCell mit ContactStatus-Badge kalt/warm/heiß/nr1). Sortierung, globale Suche, Status- + Lead-Source-Filter, Spalten-Sichtbarkeit mit localStorage-Persistenz. Klick auf Row öffnet ContactChatPanel (Sheet rechts, Plain-Text-Chat mit Enter-To-Send, Shift+Enter-Newline, Auto-Scroll zum letzten Eintrag, Edit/Delete via Hover-Icons, „(bearbeitet)"-Marker). Klick auf "Deals"-Counter navigiert zu `/leads?contact=<id>` mit Filter-Badge ("Gefiltert: [Makler] (X)") und X-Button zum Entfernen. Migration 009 öffnet contact_comments-CRUD für anon (ADR-011-konform). ADR-014: clientseitige Aggregation in useContacts statt SQL-View. ADR-015: Plain-Textarea statt Tiptap für Chat. Build grün, Bundle 377→380 KB gzipped. UI-Smoke-Test nicht im Browser durchgeführt — Verifikation via TS-Build. |
