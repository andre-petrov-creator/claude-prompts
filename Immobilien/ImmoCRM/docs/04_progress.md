...
| 7 | Akquise-Pipeline (lokaler Watcher) | 🟡 | 2026-05-15 | 017 | Architektur final via lokalem Watcher + Cloud-Briefträger (ADR-017). Cloud-Pfad end-to-end verifiziert (5 Bug-Fixes inkl. file.name URL-encoding). body.txt-Persistierung wirkt. Watcher startet headless Claude mit Modul-0-Skill im Akquise-Modus, Skill liest PDFs + body.txt. **Bug-Fix-Runde 2 (abends):** Watcher `--allowedTools "Bash Read Write Edit Glob Grep"` + deals-Schema-Match (address/city/zip statt label) + Watcher-.env mit echten Supabase-Werten + VBScript-Wrapper (kein Fenster-Flash mehr) committed. **Skill-Run verifiziert**: Adress-Extraktion + MD5-Duplikat-Detection klappt. **Noch offen:** Duplikat-Handling (Phase 1.5) + Phase 2 (KI-Klassifikation, Sub-Ordner, Link-Pipeline). Echter Lead-Insert bei nicht-Duplikat-Mail beim nächsten realen Eingang verifiziert. |
