...
- **2026-05-16**: `skills/claude-code-blueprint/` um **Phase 5: Session-Handoff via `handoff.md`** erweitert (Context-Rot-Fix nach Apex-Automate-Guide). 6-Block-Struktur (Ziel · Stand · Files · Geändert · Gescheitert · Next), Copy-Paste-Prompts für Session-Ende und Resume, neues Template unter `templates/handoff.md`. CLAUDE.md-Vorlage ergänzt um Handoff-Regel beim Session-Ende und Lese-Regel beim Session-Start. Pflicht-Übergabe zwischen je zwei Schritten im Implementierungsplan — kein Verlass mehr auf `/compact`.
